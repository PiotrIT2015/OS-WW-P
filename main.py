...
class Process:
    def __init__(self, target, args=(), name=None, is_daemon=False):
        self.id = uuid.uuid4(); self.target = target; self.args = args
        self.name = name if name else f"Process-{self.id}"; self.is_daemon = is_daemon
        self.thread = None; self.status = "waiting"; self.output = []
    def start(self):
        if self.status == "waiting":
            self.status = "running"
            self.thread = threading.Thread(target=self._run, daemon=self.is_daemon); self.thread.start()
    def _run(self):
        try: self.output.append(self.target(*self.args))
        except Exception as e: self.output.append(f"Error: {e}")
        finally: self.status = "stopped"
...
class FileSystem:
    ...
    def create_file(self, name):
        if name not in self.files: self.files[name] = File(name); return True
        return False
    def read_file(self, name): return self.files.get(name, None)
    def write_file(self, name, content):
        if name in self.files: self.files[name].content = content; return True
        return False
    def delete_file(self, name):
        if name in self.files: del self.files[name]; return True
        return False

class Scheduler:
    def __init__(self):
        self.tasks = deque(); self._run = True
        self._thread = threading.Thread(target=self._schedule, daemon=True); self._thread.start()
    def add_task(self, process, interval): self.tasks.append((process, interval, time.time()))
    def _schedule(self):
        while self._run:
            if self.tasks:
                process, interval, last_run = self.tasks.popleft()
                if time.time() - last_run >= interval:
                    process.start(); self.tasks.append((process, interval, time.time()))
                else: self.tasks.appendleft((process, interval, last_run))
            time.sleep(0.1)
...
class ImageViewerApp:
    ...
    def is_image_file(self, path): return path.lower().endswith((".png", ".jpg", ".jpeg", ".gif", ".bmp"))
    def is_text_file(self, path): return path.lower().endswith((".txt", ".log", ".py", ".md", ".json", ".csv"))

    def show_file_content(self, file_path):
        if self.is_image_file(file_path): self.show_image(file_path)
        elif self.is_text_file(file_path): self.show_text_content(file_path)
        else: self.open_with_default_app(file_path)
    ...
    def show_image(self, image_path):
        try:
            ...
            print(f"Error loading image {image_path}: {e}"); self.show_default_image()

    def show_text_content(self, file_path):
        try:
            self.canvas.pack_forget(); self.text_area.pack(fill="both", expand=True)
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
                self.text_area.delete('1.0', tk.END); self.text_area.insert('1.0', file.read())
        except Exception as e:
            self.text_area.delete('1.0', tk.END); self.text_area.insert('1.0', f"Error reading file: {e}")

    def show_default_image(self):
        if os.path.exists(self.default_image_path): self.show_image(self.default_image_path)
        else:
            self.text_area.pack_forget(); self.canvas.pack(fill="both", expand=True)
            self.canvas.delete("all")
...
class OS:
    def __init__(self, root):
        self.root = root
        self.processes = {}; self.filesystem = FileSystem(); self.scheduler = Scheduler()
        self.app_windows = {}; self.taskbar_buttons = {}; self.desktop_bg_color = "#3d405b"
        self.create_desktop(); self.create_taskbar(); self.create_icons(); self.update_time()
        self.tornado_thread = None; self.start_tornado_server()
    ...
    def create_icons(self):
        ...
        icons_data = [
            {"name": "File Manager (Sim)", "icon": "file_icon.png", "action": self.open_file_manager},
            {"name": "White War Explorer", "icon": "folder_icon.png", "action": self.open_image_viewer},
            {"name": "Stock Analyzer", "icon": "chart_icon.png", "action": self.open_stock_analyzer},
            {"name": "White Dwarf Search", "icon": "browser.png", "action": self.open_white_dwarf},
        ...
        for i, icon_data in enumerate(icons_data):
            ...
            except Exception as e: print(f"Błąd podczas tworzenia ikony '{icon_data['name']}': {e}")

    def open_image_viewer(self):
        if "image_viewer" not in self.app_windows or not self.app_windows["image_viewer"].winfo_exists():
            win = tk.Toplevel(self.root)
            self.app_windows["image_viewer"] = win
            ImageViewerApp(win)
            self.add_taskbar_button("image_viewer", "🖼️ Explorer", win)
        else: self.app_windows["image_viewer"].lift()

    def open_yii_app(self):
        try: webbrowser.open("http://yii-application.test")
        except webbrowser.Error as e: messagebox.showerror("Error", f"Nie można otworzyć przeglądarki: {e}")

    def open_file_manager(self):
        if "file_manager" not in self.app_windows or not self.app_windows["file_manager"].winfo_exists():
            win = tk.Toplevel(self.root); win.title("File Manager (Simulated)")
            self.app_windows["file_manager"] = win
            self.create_file_manager_widgets(win)
            self.add_taskbar_button("file_manager", "📁 File Sim", win)
        else: self.app_windows["file_manager"].lift()
    ...
    def open_settings(self):
        ...
        else: self.app_windows["settings"].lift()
    ...
    def open_stock_analyzer(self):
        if "stock_analyzer" not in self.app_windows or not self.app_windows["stock_analyzer"].winfo_exists():
            win = tk.Toplevel(self.root); win.title("Stock Analyzer")
            self.app_windows["stock_analyzer"] = win
            self.create_stock_analyzer_widgets(win)
            self.add_taskbar_button("stock_analyzer", "📈 Stocks", win)
        else: self.app_windows["stock_analyzer"].lift()

    def open_white_dwarf(self):
        if "white_dwarf" not in self.app_windows or not self.app_windows["white_dwarf"].winfo_exists():
            win = tk.Toplevel(self.root); win.title("White Dwarf Web Searcher"); win.geometry("700x500")
            self.app_windows["white_dwarf"] = win
            self.create_white_dwarf_widgets(win)
            self.add_taskbar_button("white_dwarf", "🔎 Web Search", win)
        else: self.app_windows["white_dwarf"].lift()

    def create_file_manager_widgets(self, parent):
        file_frame = ttk.LabelFrame(parent, text="File List"); file_frame.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
        self.file_listbox = Listbox(file_frame, width=50); self.file_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        scrollbar = ttk.Scrollbar(file_frame, orient="vertical", command=self.file_listbox.yview); scrollbar.pack(side=tk.RIGHT, fill="y")
        self.file_listbox.config(yscrollcommand=scrollbar.set); self.update_file_list()
        op_frame = ttk.LabelFrame(parent, text="Operations"); op_frame.pack(padx=10, pady=5, fill=tk.X)
        self.file_name_entry = ttk.Entry(op_frame, width=20); self.file_name_entry.pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def create_settings_widgets(self, parent):
        notebook = ttk.Notebook(parent); notebook.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
        net_frame = ttk.Frame(notebook); notebook.add(net_frame, text="Network")
        pers_frame = ttk.Frame(notebook); notebook.add(pers_frame, text="Personalization")
        ttk.Label(net_frame, text="IP Address:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        self.ip_display = ttk.Label(net_frame, text="N/A"); self.ip_display.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)
        ttk.Label(net_frame, text="Hostname:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        self.hostname_display = ttk.Label(net_frame, text="N/A"); self.hostname_display.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
    ...
    def create_stock_analyzer_widgets(self, parent):
        ...
        self.stock_entry = tk.Entry(parent, width=20); self.stock_entry.pack(pady=5); self.stock_entry.insert(0, "AAPL")
    ...
    def create_white_dwarf_widgets(self, parent):
        input_frame = Frame(parent); input_frame.pack(fill=tk.X, padx=10, pady=5)
        tk.Label(input_frame, text="Adres URL:").pack(side=tk.LEFT)
        self.dwarf_url_entry = tk.Entry(input_frame); self.dwarf_url_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self.dwarf_url_entry.insert(0, "https://www.reuters.com/technology/")
        query_frame = Frame(parent); query_frame.pack(fill=tk.X, padx=10, pady=5)
        tk.Label(query_frame, text="Szukana fraza:").pack(side=tk.LEFT)
        self.dwarf_query_entry = tk.Entry(query_frame); self.dwarf_query_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self.dwarf_query_entry.insert(0, "AI")
        action_frame = Frame(parent); action_frame.pack(fill=tk.X, padx=10, pady=5)
    ...
    def update_file_list(self):
        if hasattr(self, 'file_listbox') and self.file_listbox.winfo_exists():
            ...
            for file_name in self.filesystem.files.keys(): self.file_listbox.insert(tk.END, file_name)

    def create_new_file(self):
        name = self.file_name_entry.get()
        if name:
            if self.filesystem.create_file(name): self.update_file_list(); messagebox.showinfo("Success", f"File '{name}' created")
            else: messagebox.showerror("Error", f"File '{name}' already exists")
        else: messagebox.showerror("Error", "File name cannot be empty")
    ...
    def open_selected_file(self):
        ...
        if selected_indices:
            ...
            if file: self._show_simulated_file_content(file_name, file.content)

    def delete_selected_file(self):
        selected_indices = self.file_listbox.curselection()
        if selected_indices:
            file_name = self.file_listbox.get(selected_indices[0])
            if self.filesystem.delete_file(file_name): self.update_file_list(); messagebox.showinfo("Success", f"File '{file_name}' deleted")
    ...
    def _show_simulated_file_content(self, file_name, content):
        ...
        if win_key not in self.app_windows or not self.app_windows[win_key].winfo_exists():
            win = tk.Toplevel(self.root); win.title(f"Content: {file_name}")
            text_area = scrolledtext.ScrolledText(win, width=50, height=20); text_area.insert(tk.END, content if content is not None else "")
            text_area.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)
            self.app_windows[win_key] = win; self.add_taskbar_button(win_key, f"📄 {file_name}", win)
        else: self.app_windows[win_key].lift()

    def update_network_info(self):
        def _get_info():
            try: ip = socket.gethostbyname(socket.gethostname())
            except Exception: ip = "Not Available"
            try: hostname = socket.gethostname()
            except Exception: hostname = "Not Available"
    ...
    def choose_background_color(self):
        ...
        if color_code:
            self.desktop_bg_color = color_code; self.desktop.config(bg=self.desktop_bg_color)
            for widget in self.desktop.winfo_children():
                if isinstance(widget, tk.Label): widget.config(bg=self.desktop_bg_color)

    def search_stock(self):
        symbol = self.stock_entry.get().upper()
        if not symbol: messagebox.showerror("Error", "Stock symbol cannot be empty."); return
        self.result_box_stock.delete(1.0, tk.END); self.result_box_stock.insert(tk.END, f"Searching for {symbol}...")
        self.root.update_idletasks()
        threading.Thread(target=self._search_stock_thread, args=(symbol,), daemon=True).start()

    def _search_stock_thread(self, symbol):
        try:
            # POPRAWKA: Ujednolicono port do 8889
            ...
            else: self.result_box_stock.insert(tk.END, "Error: " + data.get("error", "Unknown error"))
        except requests.exceptions.RequestException as e:
    ...
    def _perform_web_search(self):
        ...
        if not url or not query: messagebox.showwarning("Brak danych", "Proszę podać adres URL i szukaną frazę."); return
        self.dwarf_result_box.delete(1.0, tk.END); self.dwarf_result_box.insert(tk.END, "Przetwarzanie... Proszę czekać.")
        self.dwarf_sentiment_label.config(text="Sentyment: Analizowanie..."); self.root.update_idletasks()
    ...
    def _perform_web_search_thread(self, url, query):
        try:
            ...
            self.dwarf_result_box.delete(1.0, tk.END); self.dwarf_sentiment_label.config(text="Sentyment: -")
        except Exception as e:
    ...
    def _run_pca_on_results(self):
        if "white_dwarf" not in self.app_windows or not self.app_windows["white_dwarf"].winfo_exists():
            messagebox.showinfo("Informacja", "Najpierw wykonaj wyszukiwanie, aby uzyskać tekst do analizy."); return
        results_text = self.dwarf_result_box.get("1.0", tk.END)
        if len(results_text.strip()) < 20:
            messagebox.showinfo("Informacja", "Brak wystarczającej ilości tekstu do przeprowadzenia analizy."); return
        sentences = [sentence.strip() for sentence in results_text.split('.') if sentence.strip()]
        if not sentences:
            messagebox.showinfo("Informacja", "Nie udało się podzielić tekstu na zdania do analizy."); return
    ...
    def show_stock_plot(self, file_path):
        try:
            df = pd.read_csv(file_path); df["Date"] = pd.to_datetime(df["Date"], errors='coerce')
            df["Close"] = pd.to_numeric(df["Close"].astype(str).str.replace(',', ''), errors='coerce')
            df.dropna(subset=['Date', 'Close'], inplace=True); df.sort_values(by="Date", inplace=True)
            plt.style.use('seaborn-v0_8-darkgrid')
        ...
        except Exception as e: messagebox.showerror("Error", f"Could not display plot: {e}")
    ...
    def add_taskbar_button(self, app_key, app_name, window):
        ...
        def on_close():
            button.destroy(); del self.taskbar_buttons[app_key]
            if app_key in self.app_windows: del self.app_windows[app_key]
            window.destroy()
        window.protocol("WM_DELETE_WINDOW", on_close)

    def handle_taskbar_button_click(self, window):
        if window.winfo_viewable(): window.iconify()
        else: window.deiconify(); window.lift()
    ...
    def run_tornado(self):
        try:
            ...
            # Serwer i klient używają teraz tego samego portu: 8889
            app.listen(8889)
            ...
            # POPRAWKA: Naprawiono błąd składni (brakowało '#')
            # Sprawdzamy, czy właściwy port (8889) jest zajęty
            if e.errno == 10048:
    ...
    def start_tornado_server(self):
        ...
        else: print("Serwer Tornado już działa.")
    ...
    def stop_tornado_server(self):
        ...
        else: print("Serwer Tornado nie był uruchomiony lub został już zatrzymany.")
